...
<html>
<head>
  <title>Add New cable Stock</title>
  <style>
    form {
  width: 50%;
  margin: auto;
...
<body>
  ...
  <form action="FormCableStock.php" method="POST">
...
$stmt = $conn->prepare("INSERT INTO cable_stock (rnum_serie,modele,etat,marque,date_added , caracteristique,return_value) VALUES (?, ?, ?, ?, NOW(),?,?)");
$return_value='NEW';
// Bind pacableeters to statement
...
$check_pcstock_stmt = $conn->prepare("SELECT rnum_serie FROM cable_stock WHERE rnum_serie = ?");
...
$check_pcfermerstock_stmt = $conn->prepare("SELECT rnum_serie FROM cable_stock_ferme WHERE rnum_serie = ?");
