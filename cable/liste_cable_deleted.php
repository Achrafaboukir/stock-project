...
<form method="post" action="liste_cable_deleted.php">
...
<body>
	...
	<table>
      ...
      $NSerie=$_POST['NSerie'];
			$modele=$_POST['modele'];
			$marque=$_POST['marque'];
			$query = "SELECT * FROM cable_stock_ferme where 1=1"; 
      if (!empty($NSerie)) {
			  ...
			  }// Select all data from pcstock table
			...
			while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
                ...
                echo "<td>";
                echo "<a href='restore_cable.php?rnum_serie=".$row['rnum_serie']."'>RESTORE</a>";
                echo "</td>";
				echo "</tr>";
			}
