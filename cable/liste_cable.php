...
<body>
	<form method="post" action="liste_cable.php">
	...
	<table>
		...
		<tbody>
			...
			$modele=$_POST['modele'];
      $marque=$_POST['marque'];
			$query = "SELECT * FROM cable_stock where etat='in stock'"; // Select all data from pcstock table
			if (!empty($rnum_serie)) {
      ...
      echo "<h4> $rows item";
			while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
                ...
                echo "<td>";
                echo "<a href='update_cable.php?rnum_serie=".$row['rnum_serie']."&modele=".$row['modele']."&marque=".$row['marque']."&caracteristique=".$row['caracteristique']."&etat=".$row['etat']."&Date_added=".$row['date_added']."'>Update</a>";
                echo "<a href='delete_cable.php?rnum_serie=".$row['rnum_serie']."'onclick='return confirmDelete()''>ENDOMAGER</a>";
				echo '<script>
