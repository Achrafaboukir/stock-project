...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($etat == "in use") {
    $stmt = $conn->prepare("SELECT COUNT(*) FROM ram WHERE rnum_serie = ?");
    ...
    if ($count > 0) {
        ...
        $stmt = $conn->prepare( "UPDATE ram SET modele='$modele', marque='$marque',  etat='$etat',date_use=now(),codebar='$codebar',caracteristique='$caracteristique' WHERE rnum_serie='$rnum_serie';");
        $stmt->execute();
        $stmt = $conn->prepare("UPDATE ram_stock SET modele='$modele', marque='$marque',  etat='$etat'  WHERE rnum_serie='$rnum_serie';");
        $stmt->execute();
        ...
        if($pcstock_count > 0){
            $stmt = $conn->prepare("INSERT INTO ram (rnum_serie, codebar, modele, marque, etat, date_use ,caracteristique ) VALUES ( '$rnum_serie','$codebar', '$modele', '$marque', '$etat', now(), '$caracteristique' );");
            $stmt->execute();
            $stmt = $conn->prepare("UPDATE ram_stock SET modele='$modele', marque='$marque',  etat='$etat' ,caracteristique='$caracteristique' WHERE rnum_serie='$rnum_serie';");
            $stmt->execute();
            if($stmt->execute()){
              header("Location: list_all.php");
              exit();
...
if ($etat == "in maintenance") {
    ...
    if ($count > 0) {
        $stmt = $conn->prepare("INSERT INTO suivi_maintenance_ram (rnum_serie,date_maintenance,technicien,audit_rapport)VALUES('$rnum_serie','$date_maintenance','$technicien','$audit'); ");
        $stmt->execute();
        $stmt = $conn->prepare("UPDATE ram SET etat='$etat' where rnum_serie='$rnum_serie';");
        $stmt->execute();
        header("Location: list_all.php");
    }else{
        echo 'the asset must be in use first ';
...
if ($etat == "return") {
    ...
    if ($count > 0) {
      ...
      $stmt = $conn->prepare("INSERT INTO retour_ram (rnum_serie, modele, marque,etat, date_return,caracteristique, raison_return, audit_rapport) VALUES ('$rnum_serie', '$modele', '$marque','return', '$date_retour','$caracteristique', '$raison_return', '$return_audit')");
      $stmt->execute();
        $stmt = $conn->prepare("DELETE FROM ram where rnum_serie='$rnum_serie';");
        $stmt->execute();
        $stmt = $conn->prepare("UPDATE ram_stock SET etat='in stock' , return_value='returned' where rnum_serie='$rnum_serie';");
        $stmt->execute();
        header("Location: list_all.php");
...
if($etat == "in stock"){
  $stmt = $conn->prepare("UPDATE ram_stock SET modele='$modele', marque='$marque',  etat='$etat' ,caracteristique='$caracteristique'  WHERE rnum_serie='$rnum_serie';");
      ...
      $stmt = $conn->prepare("DELETE FROM ram where rnum_serie='$rnum_serie';");
      $stmt->execute();
      $stmt = $conn->prepare("DELETE FROM suivi_maintenance_ram where rnum_serie='$rnum_serie';");
      $stmt->execute();
      $stmt = $conn->prepare("DELETE FROM retour_ram where rnum_serie='$rnum_serie';");
      $stmt->execute();
      if($stmt->execute()){
        header("Location: list_all.php");
        exit();
...
if (isset($_GET['etat'])) {
  $rnum_serie=$_GET['rnum_serie'];
    ...
    $sql = "SELECT * from ram_stock where rnum_serie='$rnum_serie'";
...
?>
<form method="post" action="update_ram.php">
...
if (isset($_GET['etat'])) {
    ...
    $rnum_serie=$_GET['rnum_serie'];
    if($etat=='in use'){
    $sql = "SELECT * from ram where rnum_serie='$rnum_serie'";
...
if (isset($_GET['etat'])) {
    ...
    $rnum_serie=$_GET['rnum_serie'];
    if($etat=='return'){
    $sql = "SELECT ram_stock.*, retour_ram.*
    FROM ram_stock
    INNER JOIN retour_ram ON ram_stock.rnum_serie = retour_ram.rnum_serie
    WHERE ram_stock.rnum_serie = '$rnum_serie';
    ";
...
<form method="post" action="update_ram.php">
    ...
    <option value="return" <?php if ($etat == 'return') echo 'selected'; ?>>return</option>
        ...
        <?php 
    $sqll = "SELECT * FROM suivi_maintenance_ram WHERE rnum_serie = $rnum_serie ORDER BY idmaintenance DESC LIMIT 1;";
