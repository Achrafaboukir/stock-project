...
<body>
	...
	<table>
      ...
      $modele=$_POST['modele'];
      $marque=$_POST['marque'];
			$query = "SELECT * FROM ram_stock where 1=1 "; // Select all data from pcstock table
			if (!empty($rnum_serie)) {
        ...
        echo "<td>".$row['return_value']."</td>";
                echo "<td>";
                echo "<a href='update_ram.php?rnum_serie=".$row['rnum_serie']."&modele=".$row['modele']."&marque=".$row['marque']."&etat=".$row['etat']."&date_added=".$row['date_added']."'>Update</a>";
                echo "<a href='delete_ram.php?rnum_serie=".$row['rnum_serie']."'onclick='return confirmDelete()''>ENDOMAGER</a>";
				echo '<script>
