...
<h1>ram retour List</h1>
<form class="form" method="post" action="list_retour_ram.php">
...
<table>
    ...
    <tbody>
        ...
        $query = "SELECT * FROM retour_ram where 1=1"; // Select all data from pcstock table
        if (!empty($rnum_serie)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
            ...
            $countQuery = "SELECT COUNT(*) AS count FROM retour_ram WHERE rnum_serie = '".$row['rnum_serie']."'";
            ...
            echo "<td>";
            $queryy = "SELECT * FROM ram_stock where rnum_serie='".$row['rnum_serie']."'"; // Select all data from pcstock table
        $resultt = mysqli_query($conn, $queryy); // Execute the query using the $connection variable
        if (!$resultt) {
            die("Query failed: " . mysqli_error($conn)); // Print error message and stop execution if the query fails
        }
        while ($roww = mysqli_fetch_assoc($resultt)) {
            echo "<a href='delete_retour_ram.php?id_return=".$row['id_return']."' onclick='return confirmDelete()'>Delete</a>";
            echo '<script>
