...
if (isset($_GET['idmaintenance'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM suivi_maintenance_ram WHERE idmaintenance = ?");
    ...
    if ($count == 0) {
        $stmt2 = $conn->prepare("UPDATE ram SET etat='in use' where rnum_serie=?");
        $stmt2->bind_param("s", $rnum_serie);
        $stmt2->execute();
    }
  
    // redirect back to the list_pc_stock.php page
    header("Location: liste_ram_stock.php");
    exit();
}
?>
