...
<h1>SOURIS retour List</h1>
<form class="form" method="post" action="list_retour_souris.php">
...
<table>
    ...
    <tbody>
        ...
        $query = "SELECT * FROM retour_souris where 1=1"; // Select all data from pcstock table
        if (!empty($NSerie)) {
            $query .= " and Nserie = '{$NSerie}'";
        }
        if (!empty($modele)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
            ...
            $countQuery = "SELECT COUNT(*) AS count FROM retour_souris WHERE Nserie = '".$row['Nserie']."'";
            ...
            echo "<td>";
            $queryy = "SELECT * FROM souris_stock where Nserie='".$row['Nserie']."'"; // Select all data from pcstock table
        $resultt = mysqli_query($conn, $queryy); // Execute the query using the $connection variable
        if (!$resultt) {
            die("Query failed: " . mysqli_error($conn)); // Print error message and stop execution if the query fails
        }
        while ($roww = mysqli_fetch_assoc($resultt)) {
            echo "<a href='delete_retour_souris.php?id_return=".$row['id_return']."' onclick='return confirmDelete()'>Delete</a>";
            echo '<script>
