...
<body>
	...
	<table>
		...
		<tbody>
			...
			$modele=$_POST['modele'];
			$marque=$_POST['marque'];
			$query = "SELECT * FROM souris_fermer_stock where 1=1"; // Select all data from pcstock table
			if (!empty($NSerie)) {
			...
			while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
                ...
                echo "<td>";
                echo "<a href='restore_souris.php?NSerie=".$row['NSerie']."'>RESTORE</a>";
                echo "</td>";
				echo "</tr>";
			}
