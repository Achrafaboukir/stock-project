...
if ($result->num_rows == 1) {
    ...
    $stmt = $conn->prepare( "INSERT INTO adaptateur_stock_ferme (rnum_serie, modele, marque, etat, date_deleted,caracteristique,return_value) VALUES ('$row[rnum_serie]', '$row[modele]', '$row[marque]', 'in stock', now(),'$row[caracteristique]','$row[return_value]');");
    $stmt->execute();
    ...
    $stmt->execute();
    $stmt = $conn->prepare("DELETE FROM adaptateur_stock where rnum_serie='$rnum_serie';");
    $stmt->execute();
