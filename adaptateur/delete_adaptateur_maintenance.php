...
if (isset($_GET['idmaintenance'])) {
    ...
    FROM suivi_maintenance_adaptateur 
    ...
    $stmt = $conn->prepare("DELETE FROM suivi_maintenance_adaptateur WHERE idmaintenance = ?");
    ...
    if ($count == 0) {
        $stmt2 = $conn->prepare("UPDATE adaptateur SET etat='in use' where rnum_serie=?");
