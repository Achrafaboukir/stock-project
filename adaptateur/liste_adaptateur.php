...
<body>
	<form method="post" action="liste_adaptateur.php">
	...
	<table>
		...
		<tbody>
			...
			$modele=$_POST['modele'];
      		$marque=$_POST['marque'];
			$query = "SELECT * FROM adaptateur_stock where etat='in stock'"; // Select all data from pcstock table
			if (!empty($rnum_serie)) {
      ...
      echo "<h4> $rows item";
			while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
                ...
                echo "<td>";
                echo "<a href='update_adaptateur.php?rnum_serie=".$row['rnum_serie']."&modele=".$row['modele']."&marque=".$row['marque']."&caracteristique=".$row['caracteristique']."&etat=".$row['etat']."&Date_added=".$row['date_added']."'>Update</a>";
