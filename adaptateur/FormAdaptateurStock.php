...
<html>
<head>
  <title>Add New adaptateur Stock</title>
  <style>
    form {
  width: 50%;
  margin: auto;
...
<body>
  ...
  <form action="FormAdaptateurStock.php" method="POST">
...
$stmt = $conn->prepare("INSERT INTO adaptateur_stock (rnum_serie,modele,etat,marque,date_added , caracteristique,return_value) VALUES (?, ?, ?, ?, NOW(),?,?)");
$return_value='NEW';
// Bind paadaptateureters to statement
...
$check_pcstock_stmt = $conn->prepare("SELECT rnum_serie FROM adaptateur_stock WHERE rnum_serie = ?");
...
$check_pcfermerstock_stmt = $conn->prepare("SELECT rnum_serie FROM adaptateur_stock_ferme WHERE rnum_serie = ?");
