...
if (isset($_GET['idmaintenance'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM suivi_maintenance_ecran WHERE idmaintenance = ?");
    ...
    if ($count == 0) {
        $stmt2 = $conn->prepare("UPDATE ecran SET etat='in use' where rnum_serie=?");
