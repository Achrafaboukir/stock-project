...
<nav class=nav>
  <ul>
    ...
    <li><a href="http://localhost/circet/ecran/list_retour_ecran.php" class="retour">return</a></li>
...
<body>
	...
	<table>
      ...
      $modele=$_POST['modele'];
      $marque=$_POST['marque'];
			$query = "SELECT * FROM ecran_stock where 1=1 "; // Select all data from pcstock table
			if (!empty($rnum_serie)) {
        ...
        echo "<td>".$row['caracteristique']."</td>";
                ...
                echo "<td>";
                echo "<a href='update_ecran.php?rnum_serie=".$row['rnum_serie']."&modele=".$row['modele']."&marque=".$row['marque']."&etat=".$row['etat']."&caracteristique=".$row['caracteristique']."&date_added=".$row['date_added']."'>Update</a>";
                echo "<a href='delete_ecran.php?rnum_serie=".$row['rnum_serie']."'onclick='return confirmDelete()''>ENDOMAGER</a>";
				echo '<script>
