...
<nav class=nav>
  <ul>
    ...
    <li><a href="list_use_ecran.php" class="in-use">In Use</a></li>
    <li><a href="list_retour_ecran.php" class="retour">return</a></li>
	<li><a href="liste_ecran.php" class="retour">stock</a></li>
	<li><a href="liste_ecran_deleted.php" class="retour">endomager</a></li>
...
<body>
	<form method="post" action="liste_ecran.php">
	...
	<table>
		...
		<tbody>
			...
			$modele=$_POST['modele'];
      $marque=$_POST['marque'];
			$query = "SELECT * FROM ecran_stock where etat='in stock'"; // Select all data from pcstock table
			if (!empty($rnum_serie)) {
      ...
      echo "<h4> $rows item";
			while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
                ...
                echo "<td>";
                echo "<a href='update_ecran.php?rnum_serie=".$row['rnum_serie']."&modele=".$row['modele']."&marque=".$row['marque']."&caracteristique=".$row['caracteristique']."&etat=".$row['etat']."&Date_added=".$row['date_added']."'>Update</a>";
