...
<form method="post" action="liste_ecran_deleted.php">
...
<body>
	...
	<table>
      ...
      $NSerie=$_POST['NSerie'];
			$modele=$_POST['modele'];
			$marque=$_POST['marque'];
			$query = "SELECT * FROM ecran_stock_fermer where 1=1"; // Select all data from pcstock table
      if (!empty($NSerie)) {
			...
			while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
                ...
                echo "<td>";
                echo "<a href='restore_ecran.php?rnum_serie=".$row['rnum_serie']."'>RESTORE</a>";
                echo "</td>";
				echo "</tr>";
			}
