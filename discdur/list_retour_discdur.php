...
<form class="form" method="post" action="list_retour_discdur.php">
...
<table>
    ...
    <tbody>
        ...
        $query = "SELECT * FROM retour_disc_dur where 1=1"; // Select all data from pcstock table
        if (!empty($dnum_serie)) {
            $query .= " and dnum_serie = '{$dnum_serie}'";
        }
        if (!empty($modele)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) { // Loop through the result and output the data
            ...
            $countQuery = "SELECT COUNT(*) AS count FROM retour_disc_dur WHERE dnum_serie = '".$row['dnum_serie']."'";
            ...
            echo "<td>";
            $queryy = "SELECT * FROM disc_dur_stock where dnum_serie='".$row['dnum_serie']."'"; // Select all data from pcstock table
        $resultt = mysqli_query($conn, $queryy); // Execute the query using the $connection variable
        if (!$resultt) {
            die("Query failed: " . mysqli_error($conn)); // Print error message and stop execution if the query fails
        }
        while ($roww = mysqli_fetch_assoc($resultt)) {
            echo "<a href='delete_retour_discdur.php?id_return=".$row['id_return']."' onclick='return confirmDelete()'>Delete</a>";
            echo '<script>
