...
<body>
  ...
  <div class="form__group">
	...
	<table>
      ...
      $modele=$_POST['modele'];
      $marque=$_POST['marque'];
			$query = "SELECT * FROM disc_dur_stock where 1=1"; // Select all data from pcstock table
			if (!empty($NSerie)) {
                $query .= " and dnum_serie LIKE '%{$NSerie}%'";
            }
            if (!empty($marque)) {
        ...
        echo "<td>".$row['return_value']."</td>";
                echo "<td>";
                echo "<a href='update_discdur.php?dnum_serie=".$row['dnum_serie']."&modele=".$row['modele']."&marque=".$row['marque']."&etat=".$row['etat']."&date_added=".$row['date_added']."'>Update</a>";
                echo "<a href='delete_discdur.php?dnum_serie=".$row['dnum_serie']."'onclick='return confirmDelete()''>ENDOMAGER</a>";
				echo '<script>
